...
jsPsych.plugins['survey-template-mod'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'survey-template-mod',
    description: '',
    parameters: {
      items: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    // mod added scale[0]
    const n  = trial.scale[0].length;           // Number of item responses
    ...
    for (var i = 0; i < trial.items.length; i++) {
      ...
      // mod added scale[i]
      var values = [];
      for (var j = 0; j < trial.scale[i].length; j++){ values.push(j); }
      ...
      if (i % trial.scale_repeat == 0) {
        ...
        for (var j = 0; j < trial.scale[i].length; j++) {
          html += `<div class="survey-template-header">${trial.scale[i][j]}</div>`;
        }
      }
